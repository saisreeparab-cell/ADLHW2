...
class BSQ(torch.nn.Module):
    def __init__(self, codebook_bits: int, embedding_dim: int):
        ...
        self._embeddings_dim = int(embedding_dim)

        self.down = torch.nn.Linear(self._embeddings_dim, self._codebook_bits, bias = True)
        self.up = torch.nn.Linear(self._codebook_bits, self._embeddings_dim, bias = True)
...
class BSQPatchAutoEncoder(PatchAutoEncoder, Tokenizer):
    ...
    def __init__(self, patch_size: int = 5, latent_dim: int = 128, codebook_bits: int = 10):
        """
        Init BSQPatchAutoEncoder class
        """
        super().__init__(patch_size=patch_size, latent_dim=latent_dim)

        self._codebook_bits = int(codebook_bits)
        self.bsq = BSQ(codebook_bits, latent_dim) # maybe change to a bottlneck not latent_dim
    ...
    def encode_index(self, x: torch.Tensor) -> torch.Tensor:
        """
        Run encode the input tensor x into a set of integer tokens
        """
        z = super().encode(x)
        b = self.bsq.encode(z)
    ...
    def decode_index(self, x: torch.Tensor) -> torch.Tensor:
        """
        Run decode the input tensor x into a set of integer tokens
        """
        b = self.bsq._index_to_code(x)
        z = self.bsq.decode(b)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        z = super().encode(x)
        b = self.bsq.encode(z)
        zt = self.bsq.decode(b)
        x_hat = super().decode(zt)
